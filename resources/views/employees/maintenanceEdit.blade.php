        ...
        <div class="col-md-12 offset-md-0">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title" style="color: red; "><i class="fas fa-tools"></i> Edit Maintenance Record</h4>
                </div>
                <hr>
                <div class="card-body">
                    <form method="POST" action="{{ route('maintenance.reschedule', ['id' => $maintenance->maintID]) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-row" style="justify-content: center">
                            <div class="form-group col-md-4">
                                ...
                                <select class="form-control{{ $errors->has('unitID') ? ' is-invalid' : '' }}" id="unitID" name="unitID" style="font-size:18px">
                                        <option value="{{ $maintenance->unitID }}">
                                            {{ $maintenance->unitName }} - {{ $maintenance->registrationNumber }}
                                        </option>
                                </select>
                                <!-- Error message -->
                                @error('unitID')
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <select class="form-control{{ $errors->has('mechanicAssigned') ? ' is-invalid' : '' }}" id="mechanicAssigned" name="mechanicAssigned" style="font-size:18px">
                                        <option value="{{ $maintenance->empID }}" >
                                            {{ $maintenance->mechanicFirstName }} {{ $maintenance->mechanicLastName }} - ({{ $maintenance->mechanicType }})
                                        </option>
                                </select>
                                <!-- Error message -->
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <input type="text" class="form-control" id="scheduleDateTime" name="scheduleDateTime" placeholder="Select Date and Time" style="background: white;  cursor: default;font-size:18px" value="{{ \Carbon\Carbon::parse($maintenance->scheduleDate)->format('Y-m-d H:i') }}">
                                <!-- Error message -->
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" min="1" class="form-control{{ $errors->has('mileage') ? ' is-invalid' : '' }}" id="mileage" name="mileage" style="font-size:25px;width:30%;" value="{{ $maintenance->mileage }}">
                            <!-- Error message -->
                            @error('mileage')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="notes" name="notes" rows="3" style="font-size:15px">{{ $maintenance->notes }}</textarea>
                        </div>

                        <!-- Scheduled By Field -->
                        <div class="form-row">
                            <div class="form-group col-md-4">
                                <label for="empID" style="color:black"><i class="fas fa-user"></i> Rescheduled By:</label>
                        ...
                        <!-- Submit Button -->
                        <button type="submit" class="btn btn-info"><i class="fas fa-save"></i> Reschedule</button>
                    </form>
                </div>
            </div>
        </div>
